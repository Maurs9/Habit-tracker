...
class EntryListTest {
    ...
    @Test
    fun testKnownEntriesMatchPreviousOrderingAfterShuffledReplacements() {
        val random = Random(20250915L)
        val entries = EntryList()
        val expectedByTimestamp = mutableMapOf<Timestamp, Entry>()
        val reference = Timestamp(1_735_689_600_000L)
        repeat(500) { index ->
            val entry = Entry(
                reference.minus(random.nextInt(3650)),
                listOf(UNKNOWN, NO, Entry.SKIP, YES_MANUAL, 1500)[index % 5],
                "Note $index"
            )
            entries.add(entry)
            expectedByTimestamp[entry.timestamp] = entry
        }
        val expected = expectedByTimestamp.values.sortedBy { it.timestamp }.reversed()
        val snapshot = entries.getKnown()
        assertEquals(expected, snapshot)
        entries.clear()
        assertEquals(expected, snapshot)
        assertEquals(emptyList(), entries.getKnown())
    }
