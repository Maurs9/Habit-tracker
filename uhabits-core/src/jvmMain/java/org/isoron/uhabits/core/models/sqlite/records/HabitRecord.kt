...
class HabitRecord {
    ...
    @field:Column(name = "section_id")
    var sectionId: Long? = null

    @field:Column(name = "reminder_times")
    ...
    fun copyFrom(model: Habit) {
        ...
        uuid = model.uuid
        tags = HabitTags.format(model.tags)
        ...
        if (model.hasReminder()) {
            ...
            reminderTimes = ReminderTimes.format(model.reminderTimes - (reminder.hour * 60 + reminder.minute))
        }
    }
    ...
    fun copyTo(habit: Habit) {
        ...
        habit.extraReminderTimes = if (habit.hasReminder()) ReminderTimes.parse(reminderTimes.orEmpty()) else emptySet()
    }
}
