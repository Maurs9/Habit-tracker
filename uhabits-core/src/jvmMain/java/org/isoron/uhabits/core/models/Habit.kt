...
data class Habit(
    ...
    val reminderTimes: Set<Int>
        get() {
            val primary = reminder ?: return emptySet()
            return ReminderTimes.normalize(extraReminderTimes + (primary.hour * 60 + primary.minute))
        }

    fun replaceReminderTimes(times: Collection<Int>) {
        val normalized = ReminderTimes.normalize(times)
        val previous = reminder
        val previousTime = previous?.let { it.hour * 60 + it.minute }
        val primary = previousTime?.takeIf { it in normalized } ?: normalized.firstOrNull()
        reminder = primary?.let { Reminder(it / 60, it % 60, previous?.days ?: WeekdayList.EVERY_DAY) }
        extraReminderTimes = normalized.filter { it != primary }.toSet()
    }
    ...
    fun isCompleted(value: Int): Boolean {
        return if (isNumerical) {
            when (targetType) {
                ...
                NumericalHabitType.AT_MOST -> value == Entry.YES_AUTO
            }
        } else {
    ...
    fun isReminderSuppressedToday(): Boolean {
        if (computedEntries.get(DateUtils.getTodayWithOffset()).value == Entry.SKIP) return true
        return isCompletedToday() && targetType != NumericalHabitType.AT_MOST
    }
