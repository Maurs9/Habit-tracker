...
abstract class DateUtils {
    companion object {
        ...
        fun getUtcTime(): Long = fixedLocalTime?.let { applyTimezone(it) } ?: System.currentTimeMillis()
        ...
        fun getUpcomingTimeInMillis(
            ...
            require(hour in 0..23 && minute in 0..59) { "Invalid reminder time: $hour:$minute" }
            val now = Instant.ofEpochMilli(getUtcTime())
            val zone = getZoneId()
            val date = now.atZone(zone).toLocalDate()
            var upcoming = date.atTime(hour, minute).atZone(zone)
            if (upcoming.toInstant() < now) upcoming = upcoming.withLaterOffsetAtOverlap()
            if (upcoming.toInstant() < now) upcoming = date.plusDays(1).atTime(hour, minute).atZone(zone)
            return upcoming.toInstant().toEpochMilli()
        }
