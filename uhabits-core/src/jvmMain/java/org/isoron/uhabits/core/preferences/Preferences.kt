...
open class Preferences(private val storage: Storage) {
    ...
    var listDensity: ListDensity
        get() {
            val stored = storage.getString("pref_list_density", ListDensity.STANDARD.name)
            val density = ListDensity.fromPersistedName(stored)
            if (stored != density.name) storage.putString("pref_list_density", density.name)
            return density
        }
        set(value) {
    ...
    var selectedTags: Set<String>
        get() = HabitTags.parse(storage.getString("pref_selected_tags", ""))
        set(tags) {
            storage.putString("pref_selected_tags", HabitTags.format(tags))
        }

    var savedHabitFilters: List<SavedHabitFilter>
        get() = storage.getString("pref_saved_habit_filters", "")
            .lineSequence()
            .filter { it.isNotEmpty() }
            .map { SavedHabitFilter.decode(it) }
            .toList()
        set(filters) {
            storage.putString("pref_saved_habit_filters", filters.joinToString("\n") { it.encode() })
        }
