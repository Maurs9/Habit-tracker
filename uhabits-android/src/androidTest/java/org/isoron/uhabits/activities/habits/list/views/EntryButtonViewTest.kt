...
class EntryButtonViewTest : BaseViewTest() {
    ...
    @Test
    fun testAccessibilityStatesAndActions() {
        view.habitName = "Read"
        view.timestamp = day(2)
        view.notes = "Before breakfast"
        val states = mapOf(
            Entry.YES_MANUAL to R.string.habit_entry_completed,
            Entry.YES_AUTO to R.string.habit_entry_automatic,
            Entry.NO to R.string.habit_entry_not_completed,
            Entry.SKIP to R.string.habit_entry_skipped,
            Entry.UNKNOWN to R.string.habit_entry_unknown
        )
        for ((value, state) in states) {
            view.value = value
            val info = AccessibilityNodeInfo.obtain()
            view.onInitializeAccessibilityNodeInfo(info)
            val description = info.contentDescription.toString()
            assertTrue(description.contains("Read"))
            assertTrue(description.contains("Before breakfast"))
            assertTrue(description.contains(targetContext.getString(state)))
            info.recycle()
        }
        for (shortToggle in listOf(false, true)) {
            prefs.isShortToggleEnabled = shortToggle
            val info = AccessibilityNodeInfo.obtain()
            view.onInitializeAccessibilityNodeInfo(info)
            val click = info.actionList.first { it.id == AccessibilityNodeInfo.ACTION_CLICK }
            val longClick = info.actionList.first { it.id == AccessibilityNodeInfo.ACTION_LONG_CLICK }
            assertEquals(
                targetContext.getString(if (shortToggle) R.string.habit_action_toggle else R.string.habit_action_edit),
                click.label
            )
            assertEquals(
                targetContext.getString(if (shortToggle) R.string.habit_action_edit else R.string.habit_action_toggle),
                longClick.label
            )
            info.recycle()
        }
