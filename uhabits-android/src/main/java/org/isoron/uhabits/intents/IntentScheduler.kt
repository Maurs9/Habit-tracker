...
@Inject constructor(
    ...
    private fun schedule(timestamp: Long, intent: PendingIntent, alarmType: Int): SchedulerResult {
        ...
        try {
            ...
            Log.e("IntentScheduler", "Exact alarm permission was revoked while scheduling", e)
            return SchedulerResult.IGNORED
        }
    ...
    override fun scheduleSnoozedReminder(reminderTime: Long, habit: Habit, timestamp: Long): SchedulerResult {
        val intent = pendingIntents.showSnoozedReminder(habit, reminderTime, timestamp)
        logReminderScheduled(habit, reminderTime)
        return schedule(reminderTime, intent, RTC_WAKEUP)
    }
    ...
    override fun cancelReminders(habitId: Long) {
        for (snoozed in listOf(false, true)) {
            pendingIntents.existingReminder(habitId, snoozed)?.let {
                manager.cancel(it)
                it.cancel()
            }
        }
