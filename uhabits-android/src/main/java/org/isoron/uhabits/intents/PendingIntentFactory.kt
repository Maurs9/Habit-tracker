...
@Inject constructor(
    ...
    fun showSnoozedReminder(habit: Habit, reminderTime: Long, timestamp: Long): PendingIntent =
        getBroadcast(
            context,
            (habit.id!! % Integer.MAX_VALUE).toInt() + 1,
            reminderAlarmIntent(habit.id!!, snoozed = true).apply {
                putExtra("timestamp", timestamp)
                putExtra("reminderTime", reminderTime)
            },
            FLAG_IMMUTABLE or FLAG_UPDATE_CURRENT
    ...
    fun existingReminder(habitId: Long, snoozed: Boolean): PendingIntent? =
        getBroadcast(
            context,
            (habitId % Integer.MAX_VALUE).toInt() + 1,
            reminderAlarmIntent(habitId, snoozed),
            FLAG_IMMUTABLE or FLAG_NO_CREATE
        )

    private fun reminderAlarmIntent(habitId: Long, snoozed: Boolean) =
        Intent(context, ReminderReceiver::class.java).apply {
            action = if (snoozed) ReminderReceiver.ACTION_SHOW_SNOOZED_REMINDER else ReminderReceiver.ACTION_SHOW_REMINDER
            data = Uri.parse("content://org.isoron.uhabits/habit/$habitId")
        }
