...
class ExportDBTask(
    ...
    override fun doInBackground() {
        filename = null
        filename = try {
            val dir = system.getFilesDir("Backups") ?: throw IOException("Backup storage unavailable")
            saveDatabaseCopy(context, dir)
        } catch (e: Exception) {
            BackupStatus(context).failed(e)
            Log.e("ExportDBTask", "Could not export database", e)
            null
        }
    }
