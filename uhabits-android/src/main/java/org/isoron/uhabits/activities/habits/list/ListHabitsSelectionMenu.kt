...
class ListHabitsSelectionMenu @Inject constructor(
    ...
    private val organizationDialogs: HabitOrganizationDialogs
) : ActionMode.Callback {
    ...
    override fun onPrepareActionMode(mode: ActionMode, menu: Menu): Boolean {
        ...
        menu.findItem(R.id.actionHabitTags).isVisible = behavior.canEdit()
        menu.findItem(R.id.actionReminderTimes).isVisible = behavior.canEdit()
            ...
            R.id.actionReminderTimes -> {
                listAdapter.selected.singleOrNull()?.id?.let { habitId ->
                    ReminderTimesDialog.newInstance(habitId)
                        .show(activity.supportFragmentManager, "reminderTimes")
                }
                mode.finish()
                return true
            }

            R.id.actionHabitTags -> {
                listAdapter.selected.singleOrNull()?.let { organizationDialogs.editTags(it) }
                mode.finish()
                return true
            }

            R.id.actionBulkSkip -> {
                organizationDialogs.bulkSkip(listAdapter.selected.toList())
                mode.finish()
                return true
            }
