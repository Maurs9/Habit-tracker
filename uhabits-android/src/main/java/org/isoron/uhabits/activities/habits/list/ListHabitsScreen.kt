...
@Inject constructor(
    ...
    private fun onOpenDocumentResult(resultCode: Int, data: Intent?) {
        ...
        try {
            ...
            val file = File.createTempFile("loop-import-", ".db", activity.cacheDir)
            tempFile = file
            activity.contentResolver.openInputStream(uri)?.use { input ->
                file.outputStream().use { input.copyTo(it) }
            } ?: throw IOException("Document provider returned no input stream")
            onImportData(file) { file.delete() }
        } catch (e: Exception) {
            tempFile?.delete()
    ...
    private fun onImportData(file: File, onFinished: () -> Unit) {
        taskRunner.execute(
            importTaskFactory.preview(file) { result ->
                ...
                result.fold(
                    onSuccess = { preview ->
                        BackupDialog.show(
                            activity,
                            preview,
                            onConfirm = { importConfirmed(file, onFinished) },
                            onCancel = onFinished
                        )
                    },
                    onFailure = {
                        activity.showMessage(activity.getString(R.string.backup_invalid_file))
                        onFinished()
                    }
                )
            }
    ...
    private fun importConfirmed(file: File, onFinished: () -> Unit) {
        taskRunner.execute(
