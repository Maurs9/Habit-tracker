...
class SubtitleCardView(context: Context, attrs: AttributeSet) : LinearLayout(context, attrs) {
    ...
    fun setState(state: SubtitleCardState) {
        ...
        binding.reminderLabel.text = if (state.reminderTimes.size > 1) {
            resources.getQuantityString(R.plurals.reminder_times_count, state.reminderTimes.size, state.reminderTimes.size)
