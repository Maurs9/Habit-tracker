...
class CheckmarkButtonView(
    ...
    override fun onInitializeAccessibilityNodeInfo(info: AccessibilityNodeInfo) {
        ...
        info.contentDescription = entryDescription()
        val click = if (preferences.isShortToggleEnabled) R.string.habit_action_toggle else R.string.habit_action_edit
        val longClick = if (preferences.isShortToggleEnabled) R.string.habit_action_edit else R.string.habit_action_toggle
        info.addAction(AccessibilityNodeInfo.AccessibilityAction(AccessibilityNodeInfo.ACTION_CLICK, context.getString(click)))
        info.addAction(AccessibilityNodeInfo.AccessibilityAction(AccessibilityNodeInfo.ACTION_LONG_CLICK, context.getString(longClick)))
    }

    private fun entryDescription(): String {
        val state = context.getString(
            when (value) {
                YES_MANUAL -> R.string.habit_entry_completed
                YES_AUTO -> R.string.habit_entry_automatic
                SKIP -> R.string.habit_entry_skipped
                NO -> R.string.habit_entry_not_completed
                else -> R.string.habit_entry_unknown
            }
        )
        return context.entryDescription(habitName, timestamp, state, notes)
    }
    ...
    fun performToggle() {
        ...
        announceForAccessibility(entryDescription())
