...
class HabitCardView(
    ...
    var habit: Habit? = null
        set(newHabit) {
            ...
            field = newHabit
            if (newHabit != null) {
                copyAttributesFrom(newHabit)
            } else {
                label.text = ""
                checkmarkPanel.habitName = ""
                numberPanel.habitName = ""
                checkmarkPanel.visibility = GONE
                numberPanel.visibility = GONE
            }
        }
    ...
    override fun onInitializeAccessibilityNodeInfo(info: AccessibilityNodeInfo) {
        ...
        // Keep the card action separate from the individually focusable daily entries.
        info.contentDescription = habit?.name
        if (isLongClickable) {
            info.addAction(
                AccessibilityNodeInfo.AccessibilityAction(
                    AccessibilityNodeInfo.ACTION_LONG_CLICK,
                    context.getString(R.string.habit_action_select)
                )
            )
        }
    }
