    ...
    override fun onInitializeAccessibilityNodeInfo(info: AccessibilityNodeInfo) {
        ...
        val state = when {
            ...
            value < 0 -> context.getString(R.string.habit_entry_unknown)
                ...
                val measurement = if (units.isBlank()) number else context.getString(R.string.habit_entry_value, number, units)
                val targetMet = if (targetType == AT_LEAST) value >= threshold else value <= threshold
                context.getString(
        ...
        info.contentDescription = context.entryDescription(habitName, timestamp, state, notes)
        val label = context.getString(R.string.habit_action_edit)
        info.addAction(AccessibilityNodeInfo.AccessibilityAction(AccessibilityNodeInfo.ACTION_CLICK, label))
        info.addAction(AccessibilityNodeInfo.AccessibilityAction(AccessibilityNodeInfo.ACTION_LONG_CLICK, label))
    }
